...
export const THEMES: Record<string, Theme> = {
  ...
  /*
    Spindle 전용. ocean-fresh는 Tailwind sky 팔레트를 그대로 쓴 값이라
    어느 앱에 붙여도 똑같아 보였다. 여기는 앱의 실제 브랜드색을 기준으로 잡는다.
    bg는 순백에 가깝게 올려 사진(바다)이 유일한 색면이 되게 하고,
    fg는 완전 검정 대신 항구 네이비를 써서 인쇄물 같은 깊이를 낸다.
  */
  "spindle-harbor": {
    id: "spindle-harbor",
    name: "Spindle Harbor",

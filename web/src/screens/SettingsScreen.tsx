  ...
  return (
    <ScreenFrame style={{ background: 'var(--l-bg)' }}>
      <header style={{ padding: '18px 20px 0' }}>
        <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--l-ink)' }}>설정</div>
      </header>
      ...
      <div className="no-scrollbar motion-card-list" style={{ flex: 1, overflowY: 'auto', padding: '16px 20px calc(110px + env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        >
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--l-ink-3)' }}>여행 모드 출발점</div>
            <div style={{ marginTop: 3, fontSize: 15, fontWeight: 700, color: 'var(--l-ink)' }}>{departure.name}</div>
          </div>
        ...
        <div style={{ padding: '16px 16px', background: '#fff', borderRadius: 18, boxShadow: '0 8px 20px -14px rgba(20,40,90,.25)' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--l-ink-3)', marginBottom: 10 }}>추천 장소까지의 이동시간</div>
        ...
        >
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--l-ink)' }}>사용법 다시 보기</div>
            <div style={{ marginTop: 3, fontSize: 11.5, fontWeight: 500, color: 'var(--l-ink-3)' }}>실제 화면에서 기능 위치를 안내해요</div>
          </div>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#c3d3ee" strokeWidth={2.4} strokeLinecap="round" aria-hidden>
            <path d="M9 6 l6 6 l-6 6" />
          </svg>
        ...
        <div style={{ padding: '16px 16px', background: 'var(--l-soft)', borderRadius: 18, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, fontWeight: 600, color: 'var(--l-ink-2)' }}>

...
export function HomeScreen({ departure, dial, onDialChange, onOpenDeparture, onSpun }: Props) {
  ...
  return (
    <ScreenFrame>
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px 0', zIndex: 2 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <svg width="24" height="24" viewBox="0 0 40 40" aria-hidden>
            <path d="M20 4 L24 16 L36 20 L24 24 L20 36 L16 24 L4 20 L16 16 Z" fill="var(--accent)" />
          </svg>
          <span style={{ fontSize: 19, fontWeight: 900, letterSpacing: -0.4 }}>Spindle</span>
        </div>
      ...
      {/* 타이틀 / 라이브 방위 */}
      <div style={{ textAlign: 'center', padding: '22px 24px 0', zIndex: 2, minHeight: 86 }}>
        {busy ? (
          <>
      ...
      {/* 나침반 원판 */}
      <div style={{ flex: 1, display: 'grid', placeItems: 'center', padding: '10px 0', zIndex: 1 }}>
        <div style={{ position: 'relative', width: 'min(78vw, 320px)' }}>
          <div
            aria-hidden
            style={{
              position: 'absolute',
      ...
      {/* 하단 컨트롤 ↔ 스핀 릴 (크로스페이드) */}
      <div style={{ position: 'relative', minHeight: 158, padding: '0 24px calc(22px + env(safe-area-inset-bottom))', zIndex: 2 }}>
        ...
        <div aria-hidden style={{ position: 'absolute', inset: '0 0 auto', top: 34, opacity: busy ? 1 : 0, transition: 'opacity .25s ease', pointerEvents: 'none' }}>

...
describe('스핀 단일 추천 → 여행 코스 브리지', () => {
  it('보고 있던 장소를 첫 장소로 고정하고 2곳 이상 코스를 만든다', () => {
    ...
    const course = buildCourseFromAnchor({ departure: nampo, budgetMinutes: Infinity, anchor })
  ...
  it('누적 이동시간은 순서를 따라 단조 증가한다', () => {
    const course = buildCourseFromAnchor({ departure: nampo, budgetMinutes: Infinity, anchor: poiOf('132190') })
    expect(course.status).toBe('ready')
    if (course.status !== 'ready') return
  ...
  it('같은 입력이면 방문 순서가 재현된다(결정적)', () => {
    const build = () => buildCourseFromAnchor({ departure: nampo, budgetMinutes: 40, anchor: poiOf('132190') })
    const a = build()
    const b = build()
    expect(a).toEqual(b)
  })
  ...
  it('단일 추천의 확장 사유(noteReason)를 코스 사유에 유지한다', () => {
    ...
    const course = buildCourseFromAnchor({ departure: nampo, budgetMinutes: Infinity, anchor: poiOf('132190'), noteReason: note })
    expect(course.status).toBe('ready')
    if (course.status !== 'ready') return
  ...
  it('이어 갈 장소가 부족하면 unavailable을 반환한다(단일 추천 유지)', () => {
    ...
    const course = buildCourseFromAnchor({ departure: nampo, budgetMinutes: 20, anchor: poiOf('126122') })

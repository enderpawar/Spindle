...
/** 지도에서 직접 찍은 출발점의 고정 id — 프리셋 목록과 구분해 한 칸만 유지한다. */
export const MAP_PICKED_ORIGIN_ID = "map-picked";

/**
 * 지도에서 찍은 좌표를 추천 엔진이 쓰는 출발점으로 감싼다.
 * 좌표는 단말 안에서만 흐르며 어떤 요청·저장소에도 실리지 않는다 (절대 원칙 1).
 */
export function pickedOriginFrom(point: GeoPoint): Departure {
  const zone = ZONES.find((z) => z.id === zoneOf(point));
  return {
    id: MAP_PICKED_ORIGIN_ID,
    name: zone ? `${zone.name} 근처` : "지도에서 고른 지점",
    desc: "지도에서 직접 찍은 출발점",
    lat: point.lat,
    lon: point.lng,
  };

...
describe("이동시간 다이얼 임계 경계값 20/40분 (DoD)", () => {
  ...
  it("경계값 정각은 포함, 초과는 제외", () => {
    expect(withinDial(est(20), "light")).toBe(true);
    expect(withinDial(est(20.01), "light")).toBe(false);
    expect(withinDial(est(40), "half")).toBe(true);
    expect(withinDial(est(40.01), "half")).toBe(false);
    expect(withinDial(est(999), "day")).toBe(true);
  });

  it("연결 미정의 존 쌍은 하루 나들이에서만 허용 (zones.md)", () => {
    expect(withinDial(est(5, false), "light")).toBe(false);
    expect(withinDial(est(5, false), "half")).toBe(false);
    expect(withinDial(est(5, false), "day")).toBe(true);
  });
});
...
describe("바다 방향 인접 확장 (DoD: 남포에서 남쪽)", () => {
  it("남쪽 부채꼴에 POI가 없으면 인접 방위로 확장하고 '바다' 사유를 낸다", () => {
    ...
    const result = recommend(input({ heading: 180, dial: "day", pois }));
  ...
  it("부채꼴에 POI는 있지만 다이얼로 전부 제외되면 사유가 달라진다", () => {
    ...
    const result = recommend(input({ heading: 0, dial: "light", pois }));

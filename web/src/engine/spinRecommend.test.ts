...
import { DEPARTURES, type DialId } from '../mock/pois'
...
describe('recommendFromSpin — 스핀 배선', () => {
  it('화면용 Recommendation 형태로 반환한다 (방위=스핀 섹터, 후보 비지 않음)', () => {
    const rec = recommendFromSpin({ heading: 90, departure: busanStation, dial: 'full', rng: seededRng(1) })
  ...
  it('시드가 같으면 결과가 재현된다', () => {
    const a = recommendFromSpin({ heading: 45, departure: busanStation, dial: 'half', rng: seededRng(7) })
    const b = recommendFromSpin({ heading: 45, departure: busanStation, dial: 'half', rng: seededRng(7) })
  ...
  it('출발점이 결과를 바꾼다 (방위는 출발점 기준으로 계산)', () => {
    const from1 = recommendFromSpin({ heading: 180, departure: busanStation, dial: 'full', rng: seededRng(3) })
    const from2 = recommendFromSpin({ heading: 180, departure: yeongdo, dial: 'full', rng: seededRng(3) })
  ...
  it("다이얼 '가볍게'는 확장이 없으면 모든 후보가 20분 이내다", () => {
    for (const departure of DEPARTURES) {
      for (let heading = 0; heading < 360; heading += 45) {
        const rec = recommendFromSpin({ heading, departure, dial: 'light' as DialId, rng: seededRng(heading + 1) })
        if (!rec.expandReason) {
  ...
  it('모든 방위·출발점·다이얼 조합에서 항상 후보가 최소 1곳 (시연 데드엔드 없음)', () => {
    for (const departure of DEPARTURES) {
      for (const dial of ['light', 'half', 'full'] as DialId[]) {
        for (let heading = 0; heading < 360; heading += 45) {
          const rec = recommendFromSpin({ heading, departure, dial, rng: seededRng(heading + 5) })
          expect(rec.candidates.length, `${departure.id}/${dial}/${heading}`).toBeGreaterThan(0)
        }
      }
    }
  })

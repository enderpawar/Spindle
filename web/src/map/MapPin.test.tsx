...
describe('MapPin', () => {
  it('큐레이션은 별, 일반 명소는 중앙 점으로 구분한다', () => {
    const curated = renderToStaticMarkup(<MapPin color="#5b7cb4" size={20} variant="curated" />)
    const standard = renderToStaticMarkup(<MapPin color="#5b7cb4" size={20} variant="standard" />)
  ...
  it('선택해도 크기는 유지하고 선택 블루와 키라인만 바꾼다', () => {
    const selected = renderToStaticMarkup(<MapPin color="#5b7cb4" size={20} variant="curated" selected />)
  ...
  it('혼잡·쾌적 의미 배지를 렌더링한다', () => {
    expect(renderToStaticMarkup(<MapPin color="#5b7cb4" size={20} status="busy" />)).toContain('map-pin__status--busy')
    expect(renderToStaticMarkup(<MapPin color="#5b7cb4" size={20} status="good" />)).toContain('map-pin__status--good')
  })
  it('코스 핀은 순번을 유지한다', () => {
    expect(renderToStaticMarkup(<MapPin color="#5b7cb4" size={26} order={3} variant="course" />)).toContain('>3</text>')
  })
})

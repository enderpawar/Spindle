 ...
 *
 * 기기 폭은 `innerWidth`가 아니라 `screen`으로 잰다 — 고정한 뒤에는 innerWidth가 390을
 * 보고하므로 되돌릴 기준이 사라진다. 짧은 변을 쓰는 이유는 가로 회전에서도 판정이 흔들리지
 * 않게 하기 위해서다(iOS의 screen.width는 회전해도 세로 기준이지만 Android는 바뀐다).
 *
...
export function viewportContentFor(shortSide: number): string {
  if (!Number.isFinite(shortSide) || shortSide <= 0 || shortSide >= DESIGN_WIDTH) {
...
function applyDesignViewport(meta: HTMLMetaElement) {
  const next = viewportContentFor(Math.min(window.screen.width, window.screen.height))

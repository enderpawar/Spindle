...
| 1.0.9 | 2026-09-07 | (아래 참조) | (아래 참조) | **스핀 버튼의 `<img>`를 CSS 배경으로 교체.** 브래킷이 32px 이미지 박스의 네 꼭짓점에 맞았고 `pointer-events: none`으로도 안 막혔다 → 히트 테스트가 아니라 **렌더 시점 이미지 분석**이다. WebKit은 CSS 배경을 분석하지 않으므로 `<img>` 요소를 없애 원인을 제거했다 |
...
**1.0.8에서 새로 볼 것** (브라우저로는 확인이 안 되는 것만)

- [ ] **스핀 버튼을 꾹 눌러도 모서리 브래킷이 안 뜬다** ← 1.0.8의 본체 (iOS Live Text)
- [ ] 스핀 버튼 탭이 정상 동작한다 (`pointer-events: none` 회귀 확인)
- [ ] 홈 히어로·별이 마스코트·명소 카드 사진을 꾹 눌러도 브래킷이 없다
- [ ] 결과 카드 대표 이미지 탭 → 갤러리가 열린다 (이 이미지는 규칙에서 제외했다 — 회귀 확인용)
...
| 항목 | 결과 |
|---|---|
| 최신 TestFlight 버전 | **1.0.8** (빌드 16, run `34131237553`, 커밋 `2ea428d`) — 다음은 `1.0.9`. 1.0.7은 롱프레스 브래킷으로 폐기 |

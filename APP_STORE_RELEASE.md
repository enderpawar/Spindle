...
현재 공개 버전은 1.1.0(2026-09-10 공개), 1.1.1(빌드 21)은 2026-09-11 심사 제출 — 아래 날짜별 기록이 최신 순이다.
...
- **2026-09-11:** **1.1.1 심사 제출 완료** (run `34556737655`, 빌드 21, `e17791f`).
  첫 제출(run `34556360374`)은 앱 설명에 `출처: ⓒ한국관광공사` 줄이 남은 채 들어가서,
  설명에서 그 줄을 빼고(사용자 결정 — AGENTS.md 원칙 6 예외) 다시 냈다. `submit` 레인이 이제
  걸려 있는 심사 제출을 먼저 취소한다(`cancel_in_progress_review`). **승인 시 자동 공개**.

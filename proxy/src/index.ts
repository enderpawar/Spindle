...
import { UPSTREAM_BUDGET_MS, fetchUpstream } from "./upstream";
...
async function resolveFirstImageUrl(
  ...
  const commonRes = await fetchUpstream(
    tourApiUrl("detailCommon2", contentId, env.TOURAPI_SERVICE_KEY),
    {},
    deadline,
  );
  ...
  const imageRes = await fetchUpstream(
    tourApiUrl("detailImage2", contentId, env.TOURAPI_SERVICE_KEY),
    {},
    deadline,
  );
...
async function handleImage(url: URL, env: Env): Promise<Response> {
  ...
  try {
    imageUrl = await resolveFirstImageUrl(contentId, env, deadline);
  } catch {
  ...
  let imgRes: Response;
  try {
    imgRes = await fetchUpstream(imageUrl, {}, deadline);
  } catch {

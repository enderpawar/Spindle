...
| 항목 | 차단 관계 |
|---|---|
| [ ] TourAPI **운영계정** 신청·인증키 발급 | **Phase 1 차단** — 키 없이는 프록시 검증 불가 |
...
**DoD**
- [ ] 여행 모드 완주: 출발점 선택 → 다이얼 → 스핀 → 추천 결과 표시
- [ ] 유닛테스트 통과: 방위 0°/360° 경계, 바다 방향(남포→남) 인접 확장 + 사유 문자열, 다이얼 임계 경계값(20/40분), 시드 고정 가중 랜덤 재현성
- [ ] 후보 0개 방위에서 확장 사유가 UI에 노출
...
**DoD**
- [ ] 결과 카드가 ui.md S4 요소를 전부 포함
- [ ] 이미지 없는 POI에서 폴백 정상 표시
- [ ] 프록시 강제 차단 상태에서 에러 UI + 재시도 동작

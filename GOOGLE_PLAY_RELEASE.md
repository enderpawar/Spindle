...
2. **R2(vc3) 게시** — 코드는 완료됐다. 남은 것은 사람 작업뿐이다:
...
⚠ **D0(테스트 시작일) 실제 날짜를 확인해 3절 E-2 일정표를 갱신할 것.** 아래 표는 8/20 가정값이다.
...
⚠ 아래 날짜는 **8/20 시작 가정**이다. 실제 시작일이 정해지면 갱신한다.

| 릴리스 | 시점 | versionCode | 주제 |
|---|---|---:|---|
| R1 | D0 (8/20) | 2 | 베이스라인 — 이미 빌드 완료 (`spindle-1.0-vc2-release.aab`) |
| **R2** | **D+4 (8/24)** | **3** | 첫 실행에서 막히는 것 |
| **R3** | **D+8 (8/28)** | **4** | 핵심 기능 품질 |
| **R4** | **D+12 (9/1)** | **5** | 실측 기반 안정화 (여유분 — R2·R3만으로 조건 충족) |
